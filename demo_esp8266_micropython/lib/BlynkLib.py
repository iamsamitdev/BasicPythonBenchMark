...
try:
    import machine
    def gettime(): return time.ticks_ms()
except ImportError:
            ...
            if cmd == MSG_RSP:
                ...
                if self.state == CONNECTING and i == 1:
                    if dlen == STA_SUCCESS:
                        ...
                        self._send(MSG_INTERNAL, 'ver', _VERSION, 'h-beat',
                                   self.heartbeat//1000, 'buff-in', self.buffin, 'dev', 'python')
                        try:
...
class Blynk(BlynkProtocol):
    ...
    def connect(self):
        try:
            ...
            raise ValueError('Connection with the Blynk server %s:%d failed' % (
